...
{ 
  ...
  try {
    ...
    if ($apiKey == $sever_apiKey) {
      $sql = "CALL add_role(:role_name, :created_date, :created_by, @result, @role_id)";
      $stmt = $this->connection->prepare($sql);
      ...
      $res = $this->connection->query("SELECT @result AS result, @role_id AS role_id")->fetch(PDO::FETCH_ASSOC);
